...
<template>
  <UBasePage>
    <div>
      <div class="mb-4 py-1 justify-center items-start dark:bg-#121212">
        <div class="px-0.5">
          <div class="text-lg">
            ---------------------T-co-------------------
          </div>
      ...
      <template v-for="(courseItem, courseIndex) of courseList" :key="courseIndex">
        <div class="flex flex-col mb-4 py-1 gap-2 justify-center dark:bg-#121212">
          ...
          <div class="flex px-4 justify-start items-center">
            ...
            <input v-model="courseItem.location" class="w-full" type="text" placeholder="你在哪里上课呀">
          </div>
          <div class="flex px-4 justify-start items-center">
            <div class="min-w-14">
              周数
    ...
    <div @touchmove.prevent>
      <div
        class="bg-gradient-to-r from-indigo-500 hover:from-violet-400 hover:to-teal-400 w-full min-h-10 transition-all ease-in-out z-100 duration-300 fixed dark:bg-#121212"
      ...
      >
        <div class="flex flex-col py-6 gap-6">
          <div class="flex font-medium text-xl px-4 justify-between items-center">
            选择上课周
            <div class="font-normal text-base text-black" @click="handleConfirmWeekActionSheet">
              确定
            </div>
          </div>
          <div class="grid p-1 gap-1 grid-cols-5 justify-center items-center">
            <template v-for="(item, index) of 20" :key="index">
              <div
                class="flex h-8 text-center text-purple-800 transition-all inline-block justify-center items-center"
                :class="clickedWeeks.includes(index + 1) ? 'bg-blue-500' : 'bg-indigo-200'"
        ...
        <div
          ...
          hover-class="bg-indigo-200 bg-opacity-50" :hover-stay-time="150" @click="showWeekActionSheet = false"
        >
          关闭
        </div>
      </div>
    ...
    <div @touchmove.prevent>
      ...
      >
        <div class="flex flex-col py-6 gap-6">
          <div class="flex font-medium text-xl px-4 justify-between items-center">
            星期几什么时候
            <div class="font-normal text-base text-black" @click="handleConfirmTimeActionSheet">
              确定
            </div>
          </div>
      ...
      <div
        class=" bg-dark-10 bg-opacity-50 transition-all top-0 right-0 bottom-0 left-0 z-90 fixed"
