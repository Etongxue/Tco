    ...
    <template v-for="(weeksTimetable, weeksIndex) of parsedCourseList" :key="weeksIndex">
      ...
      >
        <div
          ...
          :class="originalWeekIndex === weeksIndex ? 'bg-indigo-200/50 dark:!bg-op60' : currentWeekIndex === weeksIndex ? 'bg-indigo-500 bg-op80 dark:!bg-op20' : ''"
        >
            ...
            <template v-for="(weekWeekTimetable, weekWeekIndex) of weeksTimetable" :key="weekWeekIndex">
              <template v-if="weekWeekIndex < 5">
                <template v-for="(item, _idx) of weekWeekTimetable" :key="_idx">
                  <div class="rounded-full mx-auto h-1.5 w-1.5" :class="item ? 'bg-light-blue-500' : 'bg-indigo-200'" />
                </template>
              </template>
            </template>
          </div>
